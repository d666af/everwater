    ...
    <>
      <div style={ms.topBar}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button style={ms.broadcastBtn} onClick={() => setShowBroadcast(v => !v)}>
      ...
      {showBroadcast && (
        <div style={ms.formCard}>
          <div style={ms.formTitle}>Рассылка уведомлений</div>
          <div style={ms.field}>
            <div style={ms.label}>Получатели</div>
            <div style={ms.segmented}>
              {BROADCAST_TARGETS.map(t => (
                <button key={t.key}
                  style={{ ...ms.segBtn, ...(broadcastTarget === t.key ? ms.segBtnActive : {}) }}
          ...
          <div style={ms.field}>
            <div style={ms.label}>Сообщение</div>
            <textarea style={ms.textarea} rows={3} placeholder="Введите сообщение для рассылки..."
              value={broadcastText} onChange={e => setBroadcastText(e.target.value)} />
          </div>
          {sent && (
            <div style={ms.successMsg}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
          ...
          <div style={ms.formActions}>
            <button style={ms.cancelBtn} onClick={() => setShowBroadcast(false)}>Отмена</button>
            <button style={{ ...ms.saveBtn, ...(sending ? { opacity: 0.6 } : {}) }}
      ...
      {loading ? (
        ...
        <div style={ms.empty}>
          ...
          <button style={ms.addBtn} onClick={() => setShowForm(true)}>Добавить первого менеджера</button>
        </div>
      ) : (
        <div style={ms.list}>
          {managers.map((m, i) => (
