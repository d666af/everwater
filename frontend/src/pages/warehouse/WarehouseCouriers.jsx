...
import { getWarehouseCourierStats, getWarehouseStock, issueWaterToCourier, returnWaterFromCourier } from '../../api'
...
export default function WarehouseCouriers() {
  ...
  // Aggregate totals
  const totalOnHand = couriers.reduce((s, c) => s + c.on_hand, 0)
  const totalIssuedToday = couriers.reduce((s, c) => s + c.issued_today, 0)
  const totalReturnedToday = couriers.reduce((s, c) => s + c.returned_today, 0)
  const totalBottlesReturnedToday = couriers.reduce((s, c) => s + c.bottles_returned_today, 0)
  ...
  return (
    <WarehouseLayout title="Курьеры">
      ...
      {showReturn && <BatchModal title="Возврат воды" courier={showReturn} onClose={() => setShowReturn(null)} onSave={handleReturn} isReturn courierWater={showReturn.water || {}} />}
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
            <SumCard label="Курьеров" value={couriers.length} color={C} />
            <SumCard label="У них воды" value={totalOnHand} color="#1971C2" sub="шт." />
            <SumCard label="Выдано сегодня" value={totalIssuedToday} color="#E67700" sub="шт." />
            <SumCard label="Возвращено" value={totalReturnedToday} color="#2B8A3E" sub="шт." />
          </div>

          <div style={{ background: '#fff', borderRadius: 14, padding: '10px 14px', marginBottom: 12, boxShadow: '0 1px 4px rgba(0,0,0,0.04)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: 11, color: TEXT2, fontWeight: 600 }}>Бутылей 20л возвращено сегодня</div>
              <div style={{ fontSize: 18, fontWeight: 800, color: '#1971C2', lineHeight: 1, marginTop: 2 }}>{totalBottlesReturnedToday} шт.</div>
            </div>
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none">
              <path d="M9 3h6v3l2 3v9a2 2 0 01-2 2H9a2 2 0 01-2-2V9l2-3V3z" stroke="#1971C2" strokeWidth="1.5" strokeLinejoin="round"/>
              <path d="M10 13v2M14 13v2" stroke="#1971C2" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
          </div>

          {/* Courier list */}
          <div style={{ fontSize: 12, fontWeight: 700, color: TEXT2, textTransform: 'uppercase', letterSpacing: 0.5, padding: '2px 0 8px' }}>Курьеры · {couriers.length}</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {couriers.map(c => {
              const isExpanded = expanded === c.id
              return (
                <div key={c.id} style={{ background: '#fff', borderRadius: 18, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', cursor: 'pointer' }} onClick={() => setExpanded(e => e === c.id ? null : c.id)}>
                    <div style={{ width: 44, height: 44, borderRadius: '50%', background: `linear-gradient(135deg, ${C}, ${CD})`, color: '#fff', fontWeight: 800, fontSize: 18, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {(c.name || 'К')[0]}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 15, color: TEXT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</div>
                      <div style={{ fontSize: 11, color: TEXT2, marginTop: 2, display: 'flex', gap: 8 }}>
                        <span>На руках: <b style={{ color: c.on_hand > 0 ? CD : TEXT2 }}>{c.on_hand}</b></span>
                        {c.active_orders > 0 && <span>· Заказы: <b style={{ color: '#E67700' }}>{c.active_orders}</b></span>}
                        {c.cash_debt > 0 && <span>· Долг: <b style={{ color: '#E03131' }}>{Math.round(c.cash_debt / 1000)}к</b></span>}
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div style={{ borderTop: `1px solid ${BORDER}`, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {/* Activity stats grid */}
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
                        <StatBlock label="Выдано сегодня" value={c.issued_today} color="#E67700" bg="#FFF8E6" />
                        <StatBlock label="Возвращено сегодня" value={c.returned_today} color="#2B8A3E" bg="#EBFBEE" />
                        <StatBlock label="Доставок сегодня" value={c.delivered_today} color="#7048E8" bg="#F3F0FF" />
                        <StatBlock label="Бутылей 20л" value={c.bottles_returned_today} color="#1971C2" bg="#E8F4FD" />
                      </div>

                      {/* On-hand inventory */}
                      {c.on_hand > 0 && (
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, color: TEXT2, textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 6 }}>Сейчас на руках</div>
                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            {Object.entries(c.water).filter(([, v]) => v > 0).map(([product, qty]) => (
                      ...
                      {/* Delivered products today */}
                      {Object.keys(c.delivered_products || {}).length > 0 && (
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, color: TEXT2, textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 6 }}>Доставил сегодня</div>
                          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                            {Object.entries(c.delivered_products).map(([product, qty]) => (
                              <span key={product} style={{ fontSize: 12, padding: '4px 10px', borderRadius: 999, background: '#F3F0FF', color: '#7048E8', fontWeight: 600 }}>
                                {product}: {qty}
                              </span>
                            ))}
                          </div>
...
function SumCard({ label, value, color, sub }) {
  return (
    <div style={{ background: '#fff', borderRadius: 14, padding: '12px 10px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
      ...
      <div style={{ fontSize: 11, color: TEXT2, marginTop: 3, fontWeight: 500 }}>{label}</div>
    </div>
  )
}
...
function StatBlock({ label, value, color, bg }) {
  return (
    <div style={{ background: bg, borderRadius: 10, padding: '8px 10px', textAlign: 'center' }}>
      <div style={{ fontSize: 18, fontWeight: 800, color, lineHeight: 1 }}>{value}</div>
      <div style={{ fontSize: 9, color, marginTop: 3, fontWeight: 600, lineHeight: 1.2 }}>{label}</div>
    </div>
  )
}
